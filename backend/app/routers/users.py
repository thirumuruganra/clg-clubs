...
from app.services.payloads import user_profile_payload
...
def list_registered_students(
    ...
    for student in students:
        ...
        if normalized_department and normalize_text(student.department) != normalized_department:
            continue
        ...
        if normalized_query:
            searchable_values = [
                ...
                normalize_text(student.email),
                normalize_text(student.register_number),
                normalize_compact(student.register_number),
                normalize_text(student.department),
            ]
    ...
    filtered_students.sort(
        key=lambda student: (
            normalize_text(student.get("name") or student.get("email")),
            str(student["id"]),
        )
    )
...
def read_user(
    ...
    log_security_event("authz.user_profile.read", actor_user_id=current_user.id, target_user_id=user_id)
...
def update_user(
    ...
    if user_update.joined_clubs is not None:
        user.joined_clubs = json.dumps(unique_non_empty_strings(user_update.joined_clubs))
    ...
    log_security_event("authz.user_profile.updated", actor_user_id=current_user.id, target_user_id=user_id)
    return user_profile_payload(user)
